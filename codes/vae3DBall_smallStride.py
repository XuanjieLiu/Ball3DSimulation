...
class VAE(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(IMG_CHANNEL, FIRST_CH_NUM, kernel_size=3, stride=1, padding=1),
            #nn.BatchNorm2d(FIRST_CH_NUM),
            # nn.LeakyReLU(0.2, inplace=True),
            nn.ReLU(),

            nn.Conv2d(FIRST_CH_NUM, FIRST_CH_NUM, kernel_size=3, stride=1, padding=1),
        ...
        self.fc11 = nn.Linear(FIRST_CH_NUM * LAST_H * LAST_W, LATENT_CODE_NUM)
        self.fc12 = nn.Linear(FIRST_CH_NUM * LAST_H * LAST_W, LATENT_CODE_NUM)
        self.fc2 = nn.Linear(LATENT_CODE_NUM, FIRST_CH_NUM * LAST_H * LAST_W)

        self.decoder = nn.Sequential(

            nn.ConvTranspose2d(FIRST_CH_NUM, FIRST_CH_NUM, kernel_size=3, stride=1, padding=1),
            #nn.BatchNorm2d(FIRST_CH_NUM * 4),
            ...
            nn.ConvTranspose2d(FIRST_CH_NUM, IMG_CHANNEL, kernel_size=3, stride=1, padding=1),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        out3 = self.fc2(z).view(z.size(0), FIRST_CH_NUM, LAST_H, LAST_W)  # batch_s, 8, 7, 7
        next_frames = self.decoder(out3)
        return next_frames, mu, logvar
...
def test():
    ...
    for i in range(0, LATENT_CODE_NUM):
        ...
        sample = vae.decoder(vae.fc2(codes).view(BATCH_SIZE, FIRST_CH_NUM, LAST_H, LAST_W)).cpu()
...
def train(EPOCH):
    ...
    for i, (data, _) in enumerate(train_loader, 0):
        ...
        if i % log_interval == 0:
            ...
            save_image(recon_x.data.view(recon_x.size(0), IMG_CHANNEL, 120, 120), RESULT_PATH + str(EPOCH) + '_recon.png')
            save_image(data.data.view(data.size(0), IMG_CHANNEL, 120, 120), RESULT_PATH + str(EPOCH) + '_GT.png')
